...
function CustomArrow({ direction, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`absolute top-1/2 ${direction === "next" ? "right-4" : "left-4"} transform 
      -translate-y-1/2 px-6 py-5 border border-dark rounded-full hover:bg-dark hover:text-background`}
      style={{ zIndex: 10 }}
    >
      {direction === "next" ? "→" : "←"}
    </button>
        ...
        <Slider {...settings}>
          {slides.map((slide, index) => (
            <div key={index} className="flex flex-col items-center text-center p-6 rounded-lg">
              {/* Centering the image and ensuring it resizes responsively */}
              <img 
                src={slide.img} 
                alt={slide.name} 
